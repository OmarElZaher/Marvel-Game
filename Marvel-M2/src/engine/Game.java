...
public class Game extends JFrame{
	...
	public void endTurn() {
		...
		while (!turnOrder.isEmpty() && hasEffect((Champion) turnOrder.peekMin(), "Stun")  ) {
			...
			if(hasEffect(current,"SpeedUp")) {
				for(Effect e : current.getAppliedEffects()) {
					if(e.getName().equals("SpeedUp")) {
						if(e.getDuration() != 0) {
				...
				current.setMana(current.getMana() + 30);
				
			}
			else
				current.setCurrentHP(current.getCurrentHP() + 10);
